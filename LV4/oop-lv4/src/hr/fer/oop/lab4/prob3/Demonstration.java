...
/**
 * 
 * Demonstracija treceg zadatka 4. lab. vjezbe iz OOP2015.
 *
 */
public class Demonstration {
	...
	public static void main(String[] args) throws UnemployedCoachException {
		...
		System.out.println(Arrays.toString(croatia.formationPlaces));
		
		printPlayerNames(croatianSquad);

		nationalManager.pickStartingEleven((FootballPlayer player) -> player.getPlayingSkill() > 0);
		System.out.println("Ispis pocetnih "+croatia.getStartingEleven().size());
		...
		for (int i = 0; i < OFFERED_PLAYER_SIZE; i++) {
			offeredPlayers.add(new FootballPlayer(OFFERED_PLAYER_NAMES[i], DEF_COUNTRY, DEF_EMOTION, 
					FootballPlayer.MAX_PLAYING_SKILL, OFFERED_PLAYER_POSITIONS[i]));
		}
		...
		nationalManager.pickStartingEleven((FootballPlayer player) -> player.getPlayingSkill() > 0 && player.getName().equals("Dotur"));
