	...
	public void run() {
		...
		while (!data.isGameOver()) {
			try {				
				...
				System.out.format("Djelatnik %d preuzeo igraèa %s (rbr: %d) %n", num, player, data.nextId());
				// igraè razmišlja
				...
				if (r.nextBoolean()) {
					System.out.format("--->Igraè %s èeka ukljuèenje u eter%n", player);
					// igraè èeka za u eter
					Thread t = new Thread(() -> {
						try {
					...
					System.out.format("Igraè %s nije toèno odgovorio na kvalifikacijsko pitanje%n", player);							
				}
